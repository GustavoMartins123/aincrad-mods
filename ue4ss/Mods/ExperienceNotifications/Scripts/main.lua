-- Loader for ExperienceNotifications.
--
...
if type(sourceInfo) ~= "table" or type(sourceInfo.source) ~= "string" then
    error("ExperienceNotifications loader could not resolve its script source")
end
...
if type(SCRIPT_DIR) ~= "string" or SCRIPT_DIR == "" then
    error("ExperienceNotifications loader could not resolve the Scripts directory")
end
