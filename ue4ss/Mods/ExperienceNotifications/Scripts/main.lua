-- Standalone loader for this mod.
--
-- The implementation still requests the suite-level bridge path so integrated
-- installations remain compatible. This bootstrap redirects only that exact
-- request to the private bridge bundled inside this mod, then restores the
-- global dofile function immediately after the implementation has loaded.

local sourceInfo = debug.getinfo(1, "S")
if type(sourceInfo) ~= "table" or type(sourceInfo.source) ~= "string" then
    error("standalone bootstrap could not resolve its script source")
end
...
if type(SCRIPT_DIR) ~= "string" or SCRIPT_DIR == "" then
    error("standalone bootstrap could not resolve the Scripts directory")
end
...
if type(originalDofile) ~= "function" then
    error("standalone bootstrap requires Lua dofile")
end
